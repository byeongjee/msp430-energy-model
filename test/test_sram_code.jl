...
"""
function test_sram_code_no_fram_instruction_fetch()
    @testset "SRAM code execution isolation" begin
        # Compile the SRAM code benchmark
        @info "Compiling SRAM code benchmark..."
        run(`bash -c "source env.sh && make disasm FILE=examples/misc/sram_code_benchmark.c"`)
...
# Run the test
test_sram_code_no_fram_instruction_fetch()
