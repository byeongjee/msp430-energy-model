...
INLINE void bench_cmp_register_symbolic__cmp_immediate_register(void) {
  uint16_t src = 0x5678;
  uint16_t dst = 0x1234;
  REPEAT_INNER_ITERS(__asm__ volatile(
      ".rept " STR(TEXTUAL_REPT) "\n"
...
INLINE void bench_cmp_register_symbolic__cmp_immediate_indexed(void) {
  uint16_t src = 0x5678;
  uint16_t* base_dst = BASE_PTR + 8;
  REPEAT_INNER_ITERS(__asm__ volatile(
      ".rept " STR(TEXTUAL_REPT) "\n"
...
INLINE void bench_cmp_register_symbolic__cmp_immediate_symbolic(void) {
  uint16_t src = 0x5678;
  REPEAT_INNER_ITERS(__asm__ volatile(
      ".rept " STR(TEXTUAL_REPT) "\n"
      "  cmp.w %[src], sym_data\n"
...
int main(void) {
  ...
  BENCH(bench_cmp_register_symbolic__cmp_immediate_register());
  BENCH(bench_cmp_register_symbolic__cmp_immediate_indexed());
  BENCH(bench_cmp_register_symbolic__cmp_immediate_symbolic());
