...
INLINE void bench_cmp_immediate_register__cmp_register_indexed(void) {
  uint16_t dst = 0x1234;
  uint16_t src = 0x5678;
...
INLINE void bench_cmp_immediate_register__cmp_register_symbolic(void) {
  uint16_t dst = 0x1234;
  uint16_t src = 0x5678;
  REPEAT_INNER_ITERS(__asm__ volatile(
      ".rept " STR(TEXTUAL_REPT) "\n"
...
INLINE void bench_cmp_immediate_register__cmp_register_absolute(void) {
  uint16_t dst = 0x1234;
  uint16_t src = 0x5678;
  REPEAT_INNER_ITERS(__asm__ volatile(
      ".rept " STR(TEXTUAL_REPT) "\n"
...
int main(void) {
  ...
  BENCH(bench_cmp_immediate_register__cmp_register_indexed());
  BENCH(bench_cmp_immediate_register__cmp_immediate_symbolic());
  BENCH(bench_cmp_immediate_register__cmp_register_symbolic());
  BENCH(bench_cmp_immediate_register__cmp_immediate_absolute());
  BENCH(bench_cmp_immediate_register__cmp_register_absolute());
