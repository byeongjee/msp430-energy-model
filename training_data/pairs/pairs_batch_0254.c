...
INLINE void bench_mov_register_absolute__cmp_indexed_indexed(void) {
  uint16_t src = 0x5678;
  uint16_t* base_src = BASE_PTR;
  uint16_t* base_dst = BASE_PTR + 8;
  REPEAT_INNER_ITERS(__asm__ volatile(
...
INLINE void bench_mov_register_absolute__cmp_indexed_symbolic(void) {
  uint16_t src = 0x5678;
  uint16_t* base_src = BASE_PTR;
  REPEAT_INNER_ITERS(__asm__ volatile(
      ".rept " STR(TEXTUAL_REPT) "\n"
...
INLINE void bench_mov_register_absolute__cmp_indexed_absolute(void) {
  uint16_t src = 0x5678;
  uint16_t* base_src = BASE_PTR;
  REPEAT_INNER_ITERS(__asm__ volatile(
      ".rept " STR(TEXTUAL_REPT) "\n"
...
INLINE void bench_mov_register_absolute__cmp_symbolic_indexed(void) {
  uint16_t src = 0x5678;
  uint16_t* base_dst = BASE_PTR + 8;
  REPEAT_INNER_ITERS(__asm__ volatile(
      ".rept " STR(TEXTUAL_REPT) "\n"
...
int main(void) {
  ...
  BENCH(bench_mov_register_absolute__cmp_indexed_indexed());
  BENCH(bench_mov_register_absolute__cmp_indexed_symbolic());
  BENCH(bench_mov_register_absolute__cmp_indexed_absolute());
  BENCH(bench_mov_register_absolute__cmp_symbolic_register());
  BENCH(bench_mov_register_absolute__cmp_symbolic_indexed());
