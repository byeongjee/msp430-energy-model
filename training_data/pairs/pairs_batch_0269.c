...
INLINE void bench_mov_immediate_symbolic__mov_indexed_register(void) {
...
INLINE void bench_mov_immediate_symbolic__mov_indexed_indexed(void) {
  uint16_t* base_src = BASE_PTR;
  uint16_t* base_dst = BASE_PTR + 8;
  REPEAT_INNER_ITERS(__asm__ volatile(
      ".rept " STR(TEXTUAL_REPT) "\n"
...
INLINE void bench_mov_immediate_symbolic__mov_indexed_symbolic(void) {
  uint16_t* base_src = BASE_PTR;
  REPEAT_INNER_ITERS(__asm__ volatile(
      ".rept " STR(TEXTUAL_REPT) "\n"
      "  mov.w #0x1357, sym_data\n"
...
INLINE void bench_mov_immediate_symbolic__mov_indexed_absolute(void) {
  uint16_t* base_src = BASE_PTR;
  REPEAT_INNER_ITERS(__asm__ volatile(
      ".rept " STR(TEXTUAL_REPT) "\n"
      "  mov.w #0x1357, sym_data\n"
...
int main(void) {
  ...
  BENCH(bench_mov_immediate_symbolic__mov_indexed_register());
  BENCH(bench_mov_immediate_symbolic__mov_indexed_indexed());
  BENCH(bench_mov_immediate_symbolic__mov_indexed_symbolic());
  BENCH(bench_mov_immediate_symbolic__mov_indexed_absolute());
