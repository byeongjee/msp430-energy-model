...
INLINE void bench_add_register_absolute__mov_indirect_auto_symbolic(void) {
  uint16_t src = 0x5678;
...
INLINE void bench_add_register_absolute__mov_indirect_auto_absolute(void) {
  uint16_t src = 0x5678;
  uint16_t* psrc = BASE_PTR;
  REPEAT_INNER_ITERS(__asm__ volatile(
      ".rept " STR(TEXTUAL_REPT) "\n"
...
int main(void) {
  ...
  BENCH(bench_add_register_absolute__mov_indirect_auto_symbolic());
  BENCH(bench_add_register_absolute__mov_indirect_auto_absolute());
