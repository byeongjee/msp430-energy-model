...
INLINE void bench_add_indexed_register__add_immediate_symbolic(void) {
  uint16_t dst = 0x1234;
...
INLINE void bench_add_indexed_register__add_register_symbolic(void) {
  uint16_t dst = 0x1234;
  uint16_t* base = BASE_PTR;
  uint16_t src = 0x5678;
  REPEAT_INNER_ITERS(__asm__ volatile(
...
INLINE void bench_add_indexed_register__add_immediate_absolute(void) {
  uint16_t dst = 0x1234;
  uint16_t* base = BASE_PTR;
  REPEAT_INNER_ITERS(__asm__ volatile(
      ".rept " STR(TEXTUAL_REPT) "\n"
...
INLINE void bench_add_indexed_register__add_register_absolute(void) {
  uint16_t dst = 0x1234;
  uint16_t* base = BASE_PTR;
  uint16_t src = 0x5678;
  REPEAT_INNER_ITERS(__asm__ volatile(
...
int main(void) {
  ...
  BENCH(bench_add_indexed_register__add_immediate_symbolic());
  BENCH(bench_add_indexed_register__add_register_symbolic());
  BENCH(bench_add_indexed_register__add_immediate_absolute());
  BENCH(bench_add_indexed_register__add_register_absolute());
