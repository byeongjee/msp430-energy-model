...
"""
Data structure to hold training data: programs and their observed energy consumption
"""
struct TrainingData
    programs::Vector{Vector{ARMInstruction}}
...
"""
Generative model for parameter inference - ARM version
...
@gen function instruction_energy_model(instructions::Vector{ARMInstruction},
    ...
    for (i, inst) in enumerate(instructions)
        alpha, beta = get(params, inst.opcode, (2.0, 0.5))
...
@gen function parameter_inference_model(training_data::TrainingData)::Dict{Symbol,Tuple{Float64,Float64}}
    ...
    for opcode in all_opcodes
        ...
        alpha ~ gamma(2.0, 1.0)  # Prior: Gamma(2,1) gives mean=2, reasonable for energy

        # Scale parameter β (must be > 0)
        beta ~ gamma(1.0, 0.5)   # Prior: Gamma(1,0.5) gives mean=0.5, reasonable scale
    ...
    for (i, (program, observed_energy)) in enumerate(zip(training_data.programs, training_data.energies))
        ...
        observed_energy ~ normal(predicted_energy, 0.1)
    end
...
@gen function msp430_parameter_inference_model(training_data::MSP430TrainingData)::Dict{Symbol,Tuple{Float64,Float64}}
    ...
    # Generate energy observations for each program
    for (i, (program, observed_energy)) in enumerate(zip(training_data.programs, training_data.energies))
        predicted_energy ~ msp430_instruction_energy_model(program, learned_params)
...
function learn_msp430_parameters(training_data::MSP430TrainingData;
    ...
    # Run importance sampling
    (traces, log_weights) = importance_sampling(
        msp430_parameter_inference_model,
        (training_data,),
        constraints,
        n_samples
    )
    ...
    for opcode in all_opcodes
        ...
        if total_weight > 0
            ...
            # Fallback to default parameters
            learned_params[opcode] = get_msp430_energy_params(opcode)
        end
    end
...
function learn_msp430_parameters_mle(training_data::MSP430TrainingData)::Dict{Symbol,Tuple{Float64,Float64}}
    ...
    for (program, total_energy) in zip(training_data.programs, training_data.energies)
        ...
        instruction_weights = Float64[]
        for inst in program
            alpha, beta = get_msp430_energy_params(inst.opcode)
            push!(instruction_weights, alpha * beta)  # Expected value of gamma dist
        end
...
"""
Evaluate learned parameters on ARM test data
"""
...
function evaluate_msp430_parameters(learned_params::Dict{Symbol,Tuple{Float64,Float64}},
    ...
    for program in test_data.programs
        predicted_stats = predict_msp430_energy(program, learned_params, n_samples=n_samples)
        push!(predictions, predicted_stats.mean)
    end
