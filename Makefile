...
interpret: disasm ## Interpret assembly program (FILE=<file.c|file.S> [MAX_STEPS=<n>] [GRANULARITY=<g>|MODEL=<model>] [INTERCEPT_SPECIAL_CALLS=1])
	...
	ARGS=("--asm" "$(ASM_DIR)/$$BASENAME.asm" "--data" "$(ASM_DIR)/$$BASENAME.data"); \
...
test: ## Run Julia and Python test suites ([PATTERN=<regex>])
	...
	if [ -n "$(PATTERN)" ]; then \
		julia --project=. test/runtests.jl "$(PATTERN)" && JULIA_EXIT=0 || JULIA_EXIT=$$?; \
	else \
		julia --project=. test/runtests.jl && JULIA_EXIT=0 || JULIA_EXIT=$$?; \
	fi; \
	echo ""; \
	...
	echo "Test Summary"; \
	echo "========================================"; \
	if [ $$JULIA_EXIT -eq 0 ]; then echo "Julia tests:  PASSED"; else echo "Julia tests:  FAILED"; fi; \
	if [ $$PYTHON_EXIT -eq 0 ]; then echo "Python tests: PASSED"; else echo "Python tests: FAILED"; fi; \
	if [ $$JULIA_EXIT -eq 0 ] && [ $$PYTHON_EXIT -eq 0 ]; then \
		echo ""; \
